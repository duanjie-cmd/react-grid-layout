...
export type GridDragEvent = {
  ...
  bottomMaxHeight?: number, // 向下拖拽计算的最大高度
  rightMaxWidth?: number, // 向右拖拽计算的最大宽度
};
...
export function moveElement(
  ...
  // nearest collision.
  let sorted = sortLayoutItems(layout, compactType); // 排序
  const movingUp =
...
export function moveElementAwayFromCollision(
  ...
  console.log('moveElementAwayFromCollision:',compactType);
  const compactH = compactType === "horizontal";
  ...
  if (isUserAction) {
    ...
    const fakeItem: LayoutItem = {
      x: compactH ? Math.max(collidesWith.x - itemToMove.w, 0) : itemToMove.x, // 水平移动到左侧
      y: compactV ? Math.max(collidesWith.y - itemToMove.h, 0) : itemToMove.y, // 垂直移动到上方
      w: itemToMove.w,
      h: itemToMove.h,
      i: "-1"
    };
